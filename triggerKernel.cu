...
__global__ void findMaxAndAssign(float* restored, float* maximum, int rows, int cols, int ind) {
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int tid = threadIdx.x;
    int size_max = ceil_device(static_cast<float>(rows)*cols/1024);
...
__global__ void split(float* data_1, float* data_2, float* result, int size, int unit_size, int ima, int unit_num) {
    ...
    int factor = ceil_device(static_cast<float>(unit_size*unit_size)/1024);
...
int splitaug(float* input_data_1, float* input_data_2, float* result_array, int size) {
    ...
    int unit_size = 60;
    int ima = 3000;
    int ima2 = computeCeil(static_cast<float>(ima*ima)/1024)*2;
    int ima3 = computeCeil(static_cast<float>(ima2)/1024);
    int ima4 = computeCeil(static_cast<float>(ima3)/1024);
    ...
    int num_blocks = computeCeil(static_cast<float>(ima*ima)/num_threads);
    ...
    num_threads = 1024;
    num_blocks = computeCeil(static_cast<float>(ima*ima)/num_threads);
